...
if (host) {
  ...
  .autumn-home-rooms{display:grid;gap:14px}
  ...
  .autumn-home-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:9px}
  .autumn-home-card{position:relative;min-width:0;min-height:108px;border:1px solid #ffffffa0;background:linear-gradient(145deg,#fffaf6d9,#ffffffa6);box-shadow:0 8px 24px #402b3910;border-radius:17px;padding:12px;cursor:pointer;text-align:left;overflow:hidden;transition:transform .16s ease,box-shadow .16s ease,border-color .16s ease}
  .autumn-home-card:hover{transform:translateY(-1px);border-color:#de846a2b;box-shadow:0 12px 28px #402b3917}
  ...
  .autumn-home-icon{width:31px;height:31px;border-radius:11px;display:grid;place-items:center;background:#503e4a0b;font-size:16px}
  ...
  .autumn-home-name{margin-top:9px;font-size:11px;font-weight:720;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .autumn-home-state{margin-top:4px;font-size:14px;font-weight:680;letter-spacing:-.015em;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  ...
  .autumn-home-overlay{position:fixed;inset:0;z-index:80;display:none;align-items:center;justify-content:center;padding:18px;background:#2f24302b;backdrop-filter:blur(5px)}
  .autumn-home-overlay.open{display:flex}
  .autumn-home-sheet{width:min(390px,100%);border-radius:22px;background:#fffaf7f2;border:1px solid #ffffffcf;box-shadow:0 28px 78px #3a293741;padding:18px}
  ...
  @media(max-width:720px){.autumn-home-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.autumn-home-card{min-height:103px;padding:11px}.autumn-home-candidates{grid-template-columns:1fr}.autumn-home-overlay{align-items:flex-end;padding:10px}.autumn-home-sheet{border-radius:22px 22px 16px 16px}}
  `;
  ...
  discovery.innerHTML = `<div class="autumn-home-discovery-head"><b>发现新设备</b><span>只发现，不会自动授权</span></div><div class="autumn-home-candidates"></div>`;
  wrap.insertBefore(discovery, host);

  const overlay = document.createElement('div');
  overlay.className = 'autumn-home-overlay';
  overlay.innerHTML = `<section class="autumn-home-sheet" role="dialog" aria-modal="true"></section>`;
  ...
  function openDevice(device){
    ...
    sheet.querySelector('.autumn-home-close')?.addEventListener('click',()=>overlay.classList.remove('open'));
  }
  ...
  function render(home){
    ...
    const rooms=new Map();
    for(const device of devices){const room=String(device?.room||'未分区');if(!rooms.has(room))rooms.set(room,[]);rooms.get(room).push(device)}
    for(const [room,items] of rooms){
      const section=document.createElement('section');section.className='autumn-home-room';
      const roomHead=document.createElement('div');roomHead.className='autumn-home-room-head';roomHead.innerHTML=`<b>${escapeHtml(room)}</b><span>${items.length} 个设备</span>`;
      ...
      for(const device of items){
        ...
        card.addEventListener('click',()=>openDevice(device));grid.append(card);
      }
  ...
  async function discover(){
    ...
    const list=discovery.querySelector('.autumn-home-candidates');list.innerHTML='<div class="autumn-home-empty">正在读取 Home Assistant…</div>';
    try{
      const response=await fetch('/api/home/discover',{method:'POST',headers:{'X-Autumn-Companion':'1'},cache:'no-store'});
      const payload=await response.json();if(!response.ok)throw new Error(payload?.message||'discover failed');
      const candidates=Array.isArray(payload.candidates)?payload.candidates:[];list.replaceChildren();
      ...
      for(const candidate of candidates){
        ...
        const add=item.querySelector('.autumn-home-add');add.addEventListener('click',()=>authorize(candidate,add));list.append(item);
      }
      const unsupported=Number(payload.unsupported_count||0);discovery.querySelector('.autumn-home-discovery-head span').textContent=unsupported?`另有 ${unsupported} 个实体未开放 · 不会自动授权`:'只发现，不会自动授权';
    }catch{list.innerHTML='<div class="autumn-home-empty">暂时无法发现设备，请确认 Home Assistant 在线。</div>'}
    finally{button.disabled=false;button.textContent='＋ 发现新设备'}
  }
  ...
  async function authorize(candidate,button){
    ...
    try{
      ...
      const payload=await response.json();if(!response.ok)throw new Error(payload?.message||'authorize failed');
      button.textContent='已加入';await globalThis.autumnRefreshCompanionStatus?.();setTimeout(discover,250);
    }catch{button.disabled=false;button.textContent='重试'}
  }
