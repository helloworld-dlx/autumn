...
test("explicit update amends a saved record and rebuilds totals, context, and dashboard", async (t) => {
  const { root, mod } = await fixture(t); await mod.journalRecord(ordinary(), { dataRoot: root }); await mod.journalRecord(ordinary("2026-09-03", { unresolved: [] }), { dataRoot: root });
  const result = await mod.journalUpdate({ date: "2026-09-02", patch: { minutes: 60, completed: ["修正后的完成项。"], concepts: ["修正后的概念。"], unresolved: ["新的未解决项。"] } }, { dataRoot: root });
  assert.equal(result.dashboard.status, "updated"); assert.equal(result.state.total_learning_minutes, 165); assert.match((await fs.readFile(path.join(root, "logs", "2026-09-02.md"), "utf8")), /修正后的完成项/);
  assert.equal((await mod.journalRecent({ dataRoot: root })).find((item) => item.date === "2026-09-02").completed[0], "修正后的完成项。"); assert.ok(result.state.unresolved_items.some((item) => item.summary === "新的未解决项。")); await fs.access(path.join(root, "dashboard.html"));
});
test("update rejects missing date, immutable date patch, and unknown fields", async (t) => { const { root, mod } = await fixture(t); await mod.journalRecord(ordinary(), { dataRoot: root }); await assert.rejects(mod.journalUpdate({ date: "2026-09-03", patch: { minutes: 60 } }, { dataRoot: root }), /does not exist/); await assert.rejects(mod.journalUpdate({ date: "2026-09-02", patch: { date: "2026-09-03" } }, { dataRoot: root }), /unknown/); await assert.rejects(mod.journalUpdate({ date: "2026-09-02", patch: { invented: true } }, { dataRoot: root }), /unknown/); });
test("update remains saved when dashboard rebuild fails", async (t) => { const { root, mod } = await fixture(t); await mod.journalRecord(ordinary(), { dataRoot: root }); const result = await mod.journalUpdate({ date: "2026-09-02", patch: { minutes: 90 } }, { dataRoot: root, dashboardRebuild: async () => { throw new Error("failed"); } }); assert.equal(result.dashboard.status, "stale"); assert.match((await fs.readFile(path.join(root, "logs", "2026-09-02.md"), "utf8")), /1h30min/); });
