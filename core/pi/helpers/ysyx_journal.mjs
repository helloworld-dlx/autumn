...
export async function main(argv = process.argv.slice(2)) { const [action, payload = "{}", ...extra] = argv; if (extra.length) fail("unexpected arguments"); let input; try { input = JSON.parse(payload); } catch { fail("payload must be JSON"); } if (!input || typeof input !== "object" || Array.isArray(input) || Object.hasOwn(input, "dataRoot")) fail("payload is invalid"); const allowed = { journal_context: [], journal_record: null, journal_progress: [], journal_recent: ["limit"], journal_summary: ["end_date"], journal_review: ["stage"] }; if (!(action in allowed)) fail("unsupported action"); if (allowed[action] && Object.keys(input).some((key) => !allowed[action].includes(key))) fail("payload has unknown fields"); const calls = { journal_context: () => journalContext(), journal_record: () => journalRecord(input), journal_progress: () => journalProgress(), journal_recent: () => journalRecent(input), journal_summary: () => journalSummary(input), journal_review: () => journalReview(input) }; process.stdout.write(`${JSON.stringify(await calls[action](), null, 2)}\n`); }
