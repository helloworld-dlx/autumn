...
function emptyState() {
  return {
    ...
    last_manual_git_checkpoint: null, checkpoint_suggestions: [], unresolved_items: [], indexes: { tags: {}, concepts: {}, bugs: {} }
  };
}
export function parseState(source) {
  let state; try { state = JSON.parse(source); } catch { fail("state is invalid JSON"); }
  // V0.1 runtime state predates reminder history.  Reading it remains safe; the
  // field is persisted on the next successful journal mutation.
  if (!Object.hasOwn(state, "checkpoint_suggestions")) state.checkpoint_suggestions = [];
  ...
  if (!Array.isArray(state.checkpoint_suggestions) || !Array.isArray(state.unresolved_items) || !state.indexes || typeof state.indexes !== "object") fail("state collections are invalid");
  for (const suggestion of state.checkpoint_suggestions) {
    only(suggestion, new Set(["date", "substage", "reason", "suggested_message"]), "checkpoint suggestion");
    date(suggestion.date); if (!STAGES.has(suggestion.substage)) fail("checkpoint suggestion stage is invalid");
    plain(suggestion.reason, "checkpoint suggestion reason", { max: 80 }); plain(suggestion.suggested_message, "checkpoint suggestion message", { max: 180 });
  }
  return state;
}
...
function normalizeRecord(raw) {
  ...
  only(signals, new Set(["substage_completed", "tests_passed", "module_finished_verified", "bug_fixed_regression", "before_major_refactor", "before_new_stage", "stage_closure"]), "checkpoint signals");
...
function checkpointMessage(record, reasons) {
  if (reasons.includes("substage_completed") || reasons.includes("stage_closure")) {
    ...
    return `checkpoint: complete ${record.stage}`;
  }
  const summary = record.completed[0]?.replace(/[\r\n]/g, " ").slice(0, 120);
  return summary ? `checkpoint: ${record.stage} ${summary}` : `checkpoint: verified ${record.stage} checkpoint`;
}
export function recommendCheckpoint(record, previous = []) {
  const reasons = Object.entries(record.checkpoint_signals).filter(([, value]) => value).map(([key]) => key);
  if (!reasons.length) return { recommended: false, executed: false, provider: "unavailable", reasons: [], message: null };
  const reason = reasons.includes("substage_completed") ? "substage_completed" : reasons.includes("stage_closure") ? "stage_closure" : reasons[0];
  if (previous.some((item) => item.substage === record.stage && item.reason === reason)) return { recommended: false, executed: false, provider: "unavailable", reasons, message: null, suppressed: "already_suggested" };
  return { recommended: true, executed: false, provider: "unavailable", reasons, message: checkpointMessage(record, reasons), modes: ["manual"] };
}
...
export async function journalRecord(input, { dataRoot = DEFAULT_DATA_ROOT } = {}) {
  const record = normalizeRecord(input); return await locked(dataRoot, async () => {
    ...
    // Eligibility is intentionally evaluated only after the canonical log and
    // state have been persisted.  A reminder failure must not turn a saved
    // Journal entry into a failed write.
    let checkpoint;
    try {
      checkpoint = recommendCheckpoint(record, state.checkpoint_suggestions);
      if (checkpoint.recommended) {
        const reason = checkpoint.reasons.includes("substage_completed") ? "substage_completed" : checkpoint.reasons.includes("stage_closure") ? "stage_closure" : checkpoint.reasons[0];
        state.checkpoint_suggestions.push({ date: record.date, substage: record.stage, reason, suggested_message: checkpoint.message });
        await atomicWrite(layout.state, serializeState(state));
      }
    } catch {
...
export async function journalCheckpointConfirmed(input, { dataRoot = DEFAULT_DATA_ROOT } = {}) {
  ...
  if (!STAGES.has(confirmed.substage) || !confirmed.message.startsWith("checkpoint: ")) fail("checkpoint confirmation is invalid");
  return await locked(dataRoot, async () => { const state = await readState(dataRoot); state.last_manual_git_checkpoint = confirmed; await atomicWrite(paths(dataRoot).state, serializeState(state)); return { manual_checkpoint: confirmed }; });
}
function closureName(stage) { return `${stage}-closure.md`; }
function closureSummary(logs, stage) { const items = logs.filter((item) => item.stage === stage); return { start_date: items.at(0)?.date ?? null, end_date: items.at(-1)?.date ?? null, learning_days: items.length, total_minutes: items.reduce((total, item) => total + item.minutes, 0), logs: items }; }
function normalizeClosure(raw) {
  only(raw, new Set(["stage", "highlights", "revisit", "redo"]), "closure");
  const stage = plain(raw.stage, "closure stage", { max: 3 }); if (!STAGES.has(stage)) fail("closure stage is invalid");
  const highlights = strings(raw.highlights, "closure highlights", 3); if (!highlights.length) fail("closure highlights are required");
  return { stage, highlights, revisit: strings(raw.revisit ?? [], "closure revisit", 10), redo: raw.redo == null ? null : plain(raw.redo, "closure redo", { max: 600 }) };
}
function renderClosure(closure, summary, state) {
  const checkpoints = state.checkpoint_suggestions.filter((item) => item.substage === closure.stage);
  const confirmed = state.last_manual_git_checkpoint?.substage === closure.stage ? state.last_manual_git_checkpoint : null;
  const lines = [`# ${closure.stage} 阶段复盘`, "", "## 阶段时间", "", `- 开始：${summary.start_date ?? "（无日志）"}`, `- 结束：${summary.end_date ?? "（无日志）"}`, `- 学习天数：${summary.learning_days}`, `- 总时长：${formatMinutes(summary.total_minutes)}`, "", "## 最重要的收获", "", ...closure.highlights.map((item) => `- ${item}`), "", "## 仍然不牢的地方", "", ...(closure.revisit.length ? closure.revisit.map((item) => `- ${item}`) : ["- （无）"]), "", "## 如果重做一次", "", closure.redo ?? "（未回答）", "", "## 关键日志", "", ...(summary.logs.length ? summary.logs.map((item) => `- ${item.date}｜${item.stage}｜${formatMinutes(item.minutes)}`) : ["- （无）"]), "", "## 关键 Bugs", ""];
  const bugs = summary.logs.flatMap((item) => item.bugs.map((bug) => ({ date: item.date, title: bug.title })));
  lines.push(...(bugs.length ? bugs.map((item) => `- ${item.date}｜${item.title}`) : ["- （无）"]), "", "## Checkpoint", "");
  if (confirmed) lines.push(`- 用户确认：${confirmed.message}`); else if (checkpoints.length) lines.push(...checkpoints.map((item) => `- suggestion：${item.suggested_message}`)); else lines.push("- （无）");
  return `${lines.join("\n")}\n`;
}
export async function journalClosureContext({ stage, dataRoot = DEFAULT_DATA_ROOT } = {}) {
  const normalized = plain(stage, "closure stage", { max: 3 }); if (!STAGES.has(normalized)) fail("closure stage is invalid");
  const state = await readState(dataRoot); if (state.milestones[normalized].status !== "done") fail("closure requires a completed substage");
  const summary = closureSummary(await entries(dataRoot), normalized);
  return { stage: normalized, label: SUBSTAGES[normalized], ...summary, logged_concepts: summary.logs.flatMap((item) => item.concepts.map((text) => ({ date: item.date, text }))), logged_bugs: summary.logs.flatMap((item) => item.bugs.map((bug) => ({ date: item.date, ...bug }))), checkpoint_suggestions: state.checkpoint_suggestions.filter((item) => item.substage === normalized), manual_checkpoint: state.last_manual_git_checkpoint?.substage === normalized ? state.last_manual_git_checkpoint : null };
}
export async function journalClosureRecord(input, { dataRoot = DEFAULT_DATA_ROOT } = {}) {
  const closure = normalizeClosure(input);
  return await locked(dataRoot, async () => {
    const state = await readState(dataRoot); if (state.milestones[closure.stage].status !== "done") fail("closure requires a completed substage");
    const summary = closureSummary(await entries(dataRoot), closure.stage); const target = path.join(paths(dataRoot).reviews, closureName(closure.stage));
    await fs.mkdir(paths(dataRoot).reviews, { recursive: true, mode: 0o700 }); await regular(target);
    if (await fs.lstat(target).then(() => true).catch((error) => error.code === "ENOENT" ? false : Promise.reject(error))) fail("closure already exists");
    await atomicWrite(target, renderClosure(closure, summary, state));
    return { stage: closure.stage, review_path: `reviews/${closureName(closure.stage)}`, ...summary, highlights: closure.highlights, revisit: closure.revisit, redo: closure.redo };
  });
}
export async function journalClosures({ dataRoot = DEFAULT_DATA_ROOT } = {}) {
  const directory = paths(dataRoot).reviews; const names = await fs.readdir(directory).catch((error) => error.code === "ENOENT" ? [] : Promise.reject(error));
  const result = []; for (const name of names.filter((item) => /^([DC][1-6])-closure\.md$/.test(item)).sort()) { const source = await fs.readFile(path.join(directory, name), "utf8"); const stage = source.match(/^# ([DC][1-6]) 阶段复盘\n/m)?.[1]; if (!stage || !STAGES.has(stage)) fail("closure review is invalid"); result.push({ stage, source }); }
  return result;
}
